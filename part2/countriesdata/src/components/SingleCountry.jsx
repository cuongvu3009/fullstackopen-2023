...
function SingleCountry({ filteredData }) {
  ...
  useEffect(() => {
    axios(
      ...
      }&appid=${import.meta.env.VITE_REACT_APP_WEATHER}`
    )
      .then((response) => {
        ...
        return axios(
          `http://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=minutely,hourly,alerts&appid=${
            import.meta.env.VITE_REACT_APP_WEATHER
          }`
        );
      })
      .then((response) => {
